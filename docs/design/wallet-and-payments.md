...
2. **A settlement sweeper** — wakes every 15s and asks *what is due?*, not *what is oldest?*
   Catches what the inline path missed, **and retries on a backoff**.
...
### D7 — Retry backoff, because a shortfall does not resolve on its own

The first version of the sweeper retried every unpaid session every 15 seconds, indefinitely.
That is wrong on its own terms: **nothing about a wallet changes between two attempts seconds
apart**, so the second attempt is guaranteed to fail exactly like the first. The thing that
actually resolves a shortfall is a top-up — and D5 already collects the moment money lands.

Left running, it produced two failures, one loud and one silent:

| | |
|---|---|
| Churn | one Mongo transaction per session per attempt, forever. A development database reached **96,556 attempts**, worst single payment **3,878**. |
| **Starvation** | the sweep took the **oldest** unpaid sessions, capped at 25. Sessions that could never settle sat permanently at the head of the queue, so the batch was always full and **newer unpaid sessions were never looked at at all**. |

The starvation is the real defect, and the log noise is what hid it. Two changes:

- **`nextAttemptAt` on the payment**, doubling 30s → 6h. A permanently unpayable session drops
  from ~5,760 pointless attempts a day to **4**.
- **The sweep is ordered by what is due**, and sessions with no payment row at all — the case
  the sweeper genuinely exists for, where the process died between pricing and collecting — are
  taken **first**, since attempting one always creates a row and moves it out of the way.

**Nothing is ever written off.** The electricity was delivered, so the debt stays collectable
forever; it is simply not re-asked four times a minute. The 6-hour ceiling matters as much as
the curve — unbounded doubling would eventually schedule a retry years out, stranding a driver
who returns after a long gap.

### D8 — No credit: arrears block a new charge

Electricity cannot be repossessed. Once a car has drawn power the cost is sunk, so the only
moment the platform has any leverage is **before the contactor closes**. Nothing checked, which
meant the platform was not extending credit deliberately — it was extending **unlimited credit
by accident, to anyone, forever**. A development database accumulated 166 unpaid sessions across
87 drivers.

`assessArrears` is checked in two places and refuses a **new** charge:

| Where | Refusal |
|---|---|
| `startSession` | `409` naming the amount owed and the remedy |
| OCPP `Authorize` | `idTagInfo.status: "Blocked"` |

Three decisions worth defending:

**Two limits, because they are different questions.** An amount cap (₹200) asks *how exposed
are we* — one expensive session can breach it alone. A count cap (3 sessions) asks *is this
person ever paying* — many tiny debts never breach an amount cap, but the pattern is the same
problem. Either one trips the block.

**The debt must be aged (15 minutes) before it counts.** Settlement is asynchronous, so *unpaid*
and *not paying* are different claims. A session that ended thirty seconds ago is legitimately
unpaid — the debit may not have run, or it ran and the top-up landed a moment later. Counting
it immediately would refuse a driver whose money is already in flight.

**The gate is on start, never on stop.** A session already running is not interrupted over
money. Cutting power to a car mid-charge to collect a debt is both hostile and pointless, since
the energy already delivered is already owed.

It is also **self-clearing**: a top-up runs `settleOutstandingForUser`, the debt drops below
the threshold, and the driver is never waiting on an operator. And it is **derived, not stored** —
a denormalised `isBlocked` flag would need writing from every settle, top-up and refund, and
any missed write strands a paying customer at a charger.

`Blocked` rather than `Invalid` on the OCPP side is deliberate: the protocol distinguishes
*we do not recognise this credential* from *we recognise you and are refusing you*, and the
charge point shows the driver a different message for each. Collapsing both into `Invalid`
would tell a driver in arrears that their card is broken, and they would call support about the
wrong problem.
...
- **No write-off policy.** Debt is never abandoned, which is correct — but there is also no
  escalation path beyond blocking new charges, no dunning schedule, and no way for staff to
  forgive a balance.
- **The arrears thresholds are global.** ₹200 and three sessions apply to every driver on every
