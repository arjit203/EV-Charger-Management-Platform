# OCPP Gateway & Simulated Charger
...
## 5. Four orthogonal states — do not conflate them
...
| `Charger.hardwareStatus` | **Self-reported** — what the MACHINE says about itself | Gateway, from StatusNotification on **connectorId 0** |
| `Connector.status` | **Operational** — is *this plug* free/charging/faulted | Gateway, from StatusNotification on connectorId ≥ 1 |

**`StatusNotification` has two destinations, chosen by `connectorId`, and never writes
`Charger.status`.** OCPP 1.6 reports per connector because that is what a car occupies — and
reserves **connectorId 0 for the charge point itself**. There is no plug 0; it is how a machine
reports something true of the whole box, such as a ground failure or over-temperature.

This was a real gap, not a hypothetical one. The gateway originally looked connectorId 0 up as
a connector, found nothing (plug numbers start at 1), logged *"unknown connector 0"* and
answered OK. A charger could therefore announce its own failure and still be offered to drivers,
with every connector still reading `available`.

**Why a fourth field instead of writing `status`.** Letting the machine write the admin field
looks simpler and is not: it needs precedence rules — may a fault clear an operator's
`maintenance`? may an operator's `available` clear a live fault? — and whichever way those fall,
one of the two facts is destroyed. Two fields cost one extra condition in `assessStartability`
and keep both: *"I marked this for maintenance AND it is reporting a ground fault"* is a real
and common situation.

**A `Faulted` status also ends the sessions it makes impossible.** A connector fault ends the
session on that connector alone — its neighbour may be charging perfectly well. A charge-point
fault ends every session on the machine. `Unavailable` ends none: a charge point goes
unavailable in normal operation, and cutting power over a planned withdrawal would be wrong.

**When a charger goes offline we set `isOnline: false`.** Connector statuses are left alone
except for the in-flight ones (`preparing`/`charging`/`finishing`), which Module 16 releases to
`unavailable` — the charger is gone, so those are no longer knowable, and leaving one reading
`charging` was an outright lie. A plug already `available`, `occupied` or `faulted` is not made
worse by the charger leaving, so its last honest report stands. **Verified by test.**
...
> **This stub has since been replaced.** A tag is now valid only while a live session is holding
> it open, and the answer is one of three OCPP statuses rather than a boolean — `Accepted`,
> `Invalid` (we do not know this credential) or `Blocked` (we know you, and you owe money).
> The protocol draws that distinction so the charge point can show the driver a different
> message for each; collapsing both refusals into `Invalid` would tell a driver in arrears
> that their card is broken. See [Charging Sessions](charging-sessions.md) and
> [Wallet & Payments](wallet-and-payments.md).
