...
### What must be true before a charge may begin

Four preconditions guard the start path, and the **order they are checked in is part of the
design** — each answers a different kind of question, and the driver should hear the most
fundamental objection first.

| # | Question | Refusal |
|---|---|---|
| 1 | Is the hardware usable? | connector faulted, occupied, or the charger offline |
| 2 | Is the charger actually connected to the gateway? | the live socket registry, not the `isOnline` mirror |
| 3 | Is there a price? | the operator has published no active tariff |
| 4 | Does this driver already owe money? | an aged unpaid balance |

The last one is checked **last on purpose**. The first three are facts about the world; the
fourth is a fact about the person. Telling somebody they owe money for a charger that was never
going to work anyway is the wrong answer to the wrong question.

Three and four were added after this module — pricing and arrears are not session concerns, but
the start path is the only place they can be enforced, because it is the last moment before
energy flows. See [Tariffs & Pricing](tariffs-and-pricing.md) and
